...
var modulo_margen_bruto = {
    ...
    permiso:0,
    init: async (permiso) => {
        modulo_margen_bruto.permiso=permiso;
        helper.MostrarLoader();
        await modulo_margen_bruto.loadAllData();
    ...
    putEventsOnButtons: () => {
        ...
        $(".filtrar").on("change", async function (e) {
            ...
            if (!aprobado && modulo_margen_bruto.permiso == 1) {
        ...
        $("#btn_cdf_unitario").on("click", function (e) {
            ...
            if ($.fn.DataTable.isDataTable('#tabla_departamento')) {
                $("#tabla_departamento").DataTable().clear().draw();
                $("#tabla_departamento").DataTable().destroy();

            }

            if ($.fn.DataTable.isDataTable('#tabla_cdf_agrupacion')) {
                $("#tabla_cdf_agrupacion").DataTable().clear().draw();
                $("#tabla_cdf_agrupacion").DataTable().destroy();
            }

            $("#inpt_ejercicio_cdf").val($("#inpt_ejercicio").val()).trigger('change');
            $("#span_error").text("");
    ...
    loadAllData: async () => {
        ...
        await helper.ajax(url, "GET").then(async (result) => {
            if (result.success) {
    ...
    updateInputsModalCDF: () => {
        var valor = parseFloat($("#porcentaje_cdf").val());
    ...
    updateTable: (updatedCell, updatedRow, oldValue) => {
        ...
        var valor = parseDouble(updatedCell.data());
        ...
        helper.ajax(url, "POST", data).then(result => {
            if (result.success) {
                if (result.prepto_ventas == null) {
                    $("#lb_error_margen").text("El presupuesto de ventas no está cerrado");

                } else if (result.sie == null) {
                    $("#lb_error_margen").text("El sie no está cargado");
            ...
            if (result.success) {
                ...
                $('#tabla_departamento').DataTable({
                    ...
                    "footerCallback": function (row, data, start, end, display) {
                        ...
                        $(api.column(1).footer()).html('Total Coste Directo Fijo ');

                        // Update footer
                        $(api.column(2).footer()).html(
                            helper.number_format_js(total, 0, ',', '.') + '€'
                        );
                    }
                });
    ...
    getPresupuestosAprobados: async () => {
        ...
        helper.ajax(url, "POST", data).then(async (result) => {
            if (result.success) {
                if (result.prepto_ventas == null) {
                    $("#lb_error_margen").text("El presupuesto de ventas no está cerrados");
                } else if (result.prepto_produccion == null) {
                    $("#lb_error_margen").text("El presupuesto de producción no está cerrado");
                } else {
                    await modulo_margen_bruto.loadDatatable($("#inpt_ejercicio").val(), "", "", "", "", true);
                }
                /*  else if (result.prepto_produccion == null) {
    ...
    loadUltimaCargaCDF: async () => {
        ...
        helper.ajax(url, "POST", data).then(result => {
            if (result.success && result.cargada_por != null) {
                $("#span_error").text("Última aprobación: " + result.cargada_por);
    ...
    loadDatatable: async (ejercicio, planta, linea, tip_ref, agrupacion, cargar) => {
        ...
        await helper.ajax(url, "POST", data).then(result => {
            if (result.success) {
               ...
               /* if (result.mensaje == "No se ha realizado el cálculo del CDF Unitario" && cargar) {
                    $("#lb_error_margen").text(result.mensaje);
                        ...
                        { "data": "linea" },
                        {
                            render: function (data, type, row) {
                                if (row.agrupacion != null && row.agrupacion != '') {
                                    ...
                                    $("#lb_error_margen").text("Hay referencias sin configurar");
                                    return row.agrupacion;
                                }
                            }
                        },
