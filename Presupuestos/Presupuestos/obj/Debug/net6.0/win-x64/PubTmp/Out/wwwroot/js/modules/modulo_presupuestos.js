var modulo_presupuestos = {
    permiso:0,
    init: (permiso) => {
        modulo_presupuestos.permiso = permiso;
    ...
    loadEventsButtons: () => {
        $("#inp_ejer_presupuesto").on("change", async function () {
            ...
            if (modulo_presupuestos.permiso != 1) {
                $("#btn_aprobar").hide();
                $("#btn_abrir").hide();
            } else {
                if (aprobado) {
            ...
            if (result.success) {
                ...
                $('#tabla_margen_detalle').DataTable({
                    ...
                    "footerCallback": function (row, data, start, end, display) {
                        ...
                        $(api.column(4).footer()).html(helper.number_format_js((total_margen / total_euros) * 100, 2, ',', '.'));
    ...
    detallesCosteDirectoFijo: () => {
        ...
        helper.ajax(url, "POST", data).then(result => {
            if (result.success) {
                var cdf_almacen = result.cdf_almacen != null ? result.cdf_almacen : 0;
                var cdf_produccion = result.cdf_produccion != null ? result.cdf_produccion : 0;
